    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div style={cardStyle} className="overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
              <div className="flex items-center gap-3">
                <Shield size={20} style={{ color: 'var(--success-alt)' }} />
                <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)' }}>Active Policies</span>
                {totalPolicies > 0 && (
                  <span style={{ fontSize: '13px', fontWeight: 400, color: 'var(--text-disabled)' }}>({totalPolicies})</span>
                )}
              </div>
              <button
            ...
            <div>
              {policies.length === 0 ? (
                <div style={{ padding: '60px 20px', textAlign: 'center' }}>
                  <Shield size={36} style={{ color: 'var(--border)', margin: '0 auto 12px' }} />
                  <p style={{ color: 'var(--text-disabled)', fontWeight: 500 }}>No policies yet</p>
                  <p style={{ color: 'var(--border)', fontSize: '13px', marginTop: '4px' }}>Click "New Policy" to create your first policy.</p>
                </div>
              ) : policies.map((policy: any, i: number) => (
                <div key={policy.id}
                    style={{ padding: '16px 20px', borderTop: i > 0 ? '1px solid var(--surface-1)' : undefined }}
                    onMouseEnter={e => (e.currentTarget.style.background = 'var(--background-subtle)')}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                >
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                    <div className="space-y-2 flex-1">
                      <h3 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)' }}>{policy.name}</h3>
                      <p style={{ fontSize: '13px', fontWeight: 400, color: 'var(--text-tertiary)', lineHeight: 1.5 }}>{policy.description}</p>
                      <div className="flex flex-wrap items-center gap-2">
                        <span style={{
                          background: 'var(--background-subtle)', color: 'var(--text-secondary)', border: '1px solid var(--border)',
                          borderRadius: '6px', padding: '2px 8px', fontSize: '11px', fontWeight: 600,
                          textTransform: 'uppercase', letterSpacing: '0.08em',
                        }}>{policy.type}</span>
                        <code style={{
                          fontSize: '11px', color: 'var(--brand-light)', background: 'var(--brand-a10)',
                          border: '1px solid var(--brand-a20)', borderRadius: '6px',
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0" style={{
                      background: policy.status === 'Active' ? 'var(--success-a10)' : 'var(--neutral-a50)',
                      border: `1px solid ${policy.status === 'Active' ? 'var(--success-a25)' : 'var(--border)'}`,
                      borderRadius: '99px', padding: '4px 12px',
                    }}>
                      <div style={{
                        width: '6px', height: '6px', borderRadius: '50%',
                        background: policy.status === 'Active' ? 'var(--success-alt)' : 'var(--text-disabled)',
                      }} />
                      <span style={{ fontSize: '12px', fontWeight: 600, color: policy.status === 'Active' ? 'var(--success-alt)' : 'var(--text-disabled)' }}>
                        {policy.status}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <button onClick={() => setEditingPolicy(policy)}
                      ...
                      style={{ fontSize: '12px', fontWeight: 500, color: 'var(--brand-light)', background: 'var(--brand-a08)', border: '1px solid var(--brand-a20)' }}
                      onMouseEnter={e => (e.currentTarget.style.background = 'var(--brand-a15)')}
                      onMouseLeave={e => (e.currentTarget.style.background = 'var(--brand-a08)')}
                    ><Edit2 size={13} /> Edit</button>
                    <button onClick={() => togglePolicyStatus(policy.id)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all"
                      style={{
                        fontSize: '12px', fontWeight: 500,
                        color: policy.status === 'Active' ? 'var(--danger)' : 'var(--success-alt)',
                        background: policy.status === 'Active' ? 'var(--danger-a08)' : 'var(--success-a08)',
                        border: `1px solid ${policy.status === 'Active' ? 'var(--danger-a20)' : 'var(--success-a20)'}`,
                      }}
        ...
        <div style={{ ...cardStyle, padding: '20px' }}>
          <h3 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '20px' }}>Policy Statistics</h3>

          <div style={{ textAlign: 'center', paddingBottom: '20px', marginBottom: '20px', borderBottom: '1px solid var(--surface-1)' }}>
            <p style={{ fontSize: '48px', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1 }}>{implementationPct}%</p>
            <p style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-disabled)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '8px' }}>Policy Implementation</p>
            <p style={{ fontSize: '12px', color: 'var(--border)', marginTop: '4px' }}>{activePolicies} of {totalPolicies} enforced</p>
          </div>

          <div className="space-y-5">
            <div>
              <div className="flex justify-between items-center mb-2">
                <span style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-tertiary)' }}>Violations Today</span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--danger)' }}>{violationsToday}</span>
              </div>
              <div style={{ height: '6px', background: 'var(--surface-1)', borderRadius: '99px', overflow: 'hidden' }}>
                <div style={{
                  height: '100%', background: 'var(--danger)', borderRadius: '99px',
