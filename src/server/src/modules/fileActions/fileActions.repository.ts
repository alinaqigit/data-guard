...
export class fileActionsRepository {
  ...
  public storeEncryptedFile(data: {
    ...
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(data.userId, data.originalPath, data.encryptedPath, data.encryptionKey, data.iv);

    return this.getById(result.lastInsertRowid as number)!;
  }

  public getByEncryptedPath(encryptedPath: string, userId: number): EncryptedFileRecord | null {
  ...
  public deleteByEncryptedPath(encryptedPath: string, userId: number): void {
    this.db.prepare(
  ...
  private getById(id: number): EncryptedFileRecord | null {
    const row = this.db.prepare("SELECT * FROM encrypted_files WHERE id = ?").get(id) as any;
    return row ? this.mapRow(row) : null;
  }
