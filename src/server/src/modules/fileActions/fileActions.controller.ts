  ...
  private mapRoutes() {
    ...
    this.router.post(
      ...
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          ...
          const result = this.service.quarantineFile(
            req.user.userId,
            filePath,
          );
    ...
    this.router.post(
      ...
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          ...
          if (!filePath)
            throw new ValidationError("filePath is required");
          const result = this.service.encryptFile(
            req.user.userId,
            filePath,
          );
          res.status(200).json(result);
    ...
    this.router.post(
      ...
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          ...
          if (!filePath)
            throw new ValidationError("filePath is required");
          const result = this.service.decryptFile(
            req.user.userId,
            filePath,
          );
          res.status(200).json(result);
    ...
    this.router.post(
      ...
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          ...
          if (!filePath)
            throw new ValidationError("filePath is required");
          const result = this.service.deleteFile(
            req.user.userId,
            filePath,
          );
          res.status(200).json(result);
    ...
    this.router.get(
      ...
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          if (!req.user) throw new UnauthorizedError();
          const files = this.service.getEncryptedFiles(
            req.user.userId,
          );
    ...
    this.router.post(
      ...
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          ...
          if (!filePath)
            throw new ValidationError("filePath is required");
          const result = await this.service.openFile(filePath);
          res.status(200).json(result);
        },
      ),
    );

    // POST /api/files/show-in-folder — reveal file in system file explorer
    this.router.post(
      "/show-in-folder",
      asyncHandler(
        async (req: AuthenticatedRequest, res: Response) => {
          ...
          if (!filePath)
            throw new ValidationError("filePath is required");
          const result = await this.service.showInFolder(filePath);
          res.status(200).json(result);
        },
      ),
    );
